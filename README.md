...
本人纯新手连GitHub上面很多基础操作都不会，慢慢学习中，不要喷我，这次做这个也只是为了方便自己选图。
...
- Ctrl+Z 撤销 / Ctrl+Shift+Z 重做，工具栏撤销/重做按钮

### 直方图

- RGB 三通道 + Luma 感知亮度实时直方图
- jpeg-decoder 直接解码，6000×4000 图片 < 500ms
- 150ms 防抖 + 50 条缓存，快速翻图不卡顿
...
- 基于 EXIF 拍摄时间精确判定连拍（≤1s），EXIF 缺失时回退文件时间
- 网格视图展平显示每张 + 连拍角标 `1/3`，蓝色边框区分
...
- 光标处精准缩放（Alt+滚轮 10% / 纯滚轮 1%），Z 键重置缩放
- 鼠标拖拽平移
...
- Ctrl+Z 撤销 / Ctrl+Shift+Z 重做（工具栏按钮 + 快捷键）
- Tab 切换网格/胶片视图
